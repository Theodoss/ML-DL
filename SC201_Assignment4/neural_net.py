...
class TwoLayerNet(object):
    ...
    def cost(self, X, y=None):
        ...
        H = np.exp(F)/(np.sum(np.exp(F),axis= 1, keepdims= True))
        # L
        ...
        sigma[range(N), y] -= 1
        dF = sigma / N # note: cost function has factor 1/N
        #dW2(H, C) = dF(N, C).A(N, H)
        grads['W2'] = A.T.dot(dF)
        #db2(C,) = dF(N, C).1
        grads['b2'] = np.sum(dF,axis =0)
        #dA(N, H) = dF(N, C).W2(H, C)
        dA = dF.dot(W2.T)
        #dK = dA if(0,k)
        dK = dA*np.where(K>0,1,0)
        #dW1(D,H) = dK(N,H).X(N,D)
        grads['W1'] = X.T.dot(dK)
        #db1(H,) = dK.1
