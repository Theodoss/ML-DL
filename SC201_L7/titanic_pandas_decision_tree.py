...
def main():
    ...
    ################
    #     TODO:    #
    d_tree = tree.DecisionTreeClassifier(max_depth=10 , min_impurity_decrease= 10)
    d_tree_classifier = d_tree.fit(x_train, y)
    acc = d_tree_classifier.score(x_train, y)
    print('Train Acc:',acc)
    tree.export_graphviz(d_tree_classifier,feature_names= feature_names,out_file= 'tree_fig')
