...
def increment(d1, scale, d2):
    ...
    for i in d2:
       d1[i] = d1.get(i,0)+d2.get(i)*scale
    return d1
...
def dotProduct(d1, d2):  # D2小 D1大
    ...
    if len(d1) < len(d2):
        ...
        ## '%.7f'%float無條件捨去,轉為str 解決浮點數不精確問題
        # return float('%.7f'%sum(d1[i]*d2[i] for i in d1 if i in d2))
        return sum(d1[i] * d2[i] for i in d2 if i in d1)
