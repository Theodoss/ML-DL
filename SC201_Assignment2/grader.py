...
def test4a2():
    ...
    outputErrorAnalysis(validationExamples, featureExtractor, weights, 'error-analysis')  # Use this to debug #需要講解
    trainError = evaluatePredictor(trainExamples, lambda x : (1 if dotProduct(featureExtractor(x), weights) > 0 else -1)) # 需要講解
...
# extractWordFeatures: train error = 0.03601575689364097, validation error = 0.26730444569499157
# extractCharacterFeatures: train error = 0.0, validation error = 0.27687113111986494
# 新的Feature 產生器產生了過多的"無用"Feature，導致模型過擬合，雖然train error = 0.0 但validation反而上升
# 這告訴我們還是要稍微了解資料背景意義，並根據意義產生Feature會比較有效
