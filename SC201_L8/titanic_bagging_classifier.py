...
def main():
    ...
    #                 #
    ###################
    bagging = ensemble.BaggingClassifier(base_estimator=tree.DecisionTreeClassifier(max_depth=5, max_leaf_nodes=6),n_estimators=100)
    # bagging = ensemble.BaggingClassifier(base_estimator= linear_model.LogisticRegression(max_iter=10000))
    bagging_classifier = bagging.fit(x_train , y)
    print('Train Acc:', bagging_classifier.score(x_train, y))
    ...
    out_file(predictions, 'bagging_tree_bagging100_sklearn.csv')
